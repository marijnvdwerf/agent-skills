...
IMPORTANT: After ANY changes to the YAML configuration, you must run `python configure.py --clean` to regenerate the build files before rebuilding. This includes adding subsegments, renaming files, adding bss_size, or changing segment properties.
...
### Configuring BSS Section

Most N64 ROMs have a BSS (uninitialized data) section. Check the entrypoint asm file (usually `asm/1000.s` - splat names files by ROM offset, not `entry.s`) for BSS_START and BSS_SIZE references.

NEVER edit asm files to fix address mismatches - they are regenerated by splat and changes are lost.

If the entrypoint references `main_BSS_START`, calculate `bss_size` from the assembly:

Example calculation (your values will differ):

```asm
lui  $t0, 0x800e        # BSS_START high = 0x800e
lui  $t1, (0x7DAC0 >> 16)  # BSS_SIZE high
addiu $t0, $t0, 0x2e00  # BSS_START low = 0x2e00 → BSS_START = 0x800e2e00
ori  $t1, $t1, (0x7DAC0 & 0xFFFF)  # BSS_SIZE = 0x7DAC0
```

Add `bss_size` to the main segment and a `bss` subsegment in YAML (example values shown, calculate from YOUR entrypoint):
...
- name: main
  ...
  start: 0x1050
  vram: 0x80001050
  bss_size: 0x7DAC0  # Example - use YOUR calculated value
  subsegments:
    - [0x1050, asm]
    - { start: 0xB1BD0, type: bss, vram: 0x800E2E00 }  # Example - use YOUR BSS_START
```
...
2. Run `python configure.py --clean` and rebuild to verify still matches
...
4. Run `python configure.py --clean` and **rebuild** to verify still matches
...
4. Run `python configure.py --clean` (symbol updates require re-dumping assembly)
...
Change file type from `asm` to `c` in splatfile, then run `python configure.py --clean`. This generates:
